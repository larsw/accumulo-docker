...
# We need to check if this is a busybox-based distro (alpine) in order
# to use the right mktemp template format.                            
if [ $(readlink `which /bin/mktemp`) == "/bin/busybox" ]; then
